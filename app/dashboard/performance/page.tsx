    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {subscription ? (
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border ${
            ...
            {subscription.daysLeft <= 7 ? '⚠️' : ''}
            <span>Pack <strong>{subscription.packName}</strong> — {subscription.daysLeft}j restants</span>
          </div>
        ) : (
          <Link href="/packs" className="px-4 py-2 bg-tef-blue text-white text-sm font-semibold rounded-xl hover:bg-blue-800 transition-colors">
            Activer un pack →
          </Link>
        )}
      </div>
      ...
      {!hasAnyData && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-10 text-center space-y-4">
          ...
          <Link href="/dashboard" className="inline-block px-5 py-2.5 bg-tef-blue text-white font-semibold text-sm rounded-xl hover:bg-blue-800 transition-colors">
            Voir les séries →
          </Link>
        </div>
      )}
