// Script to seed CO Série 11 into the database
// Run: node scripts/seed-co-serie-11.js
...
async function main() {
  ...
  try {
    ...
    const existing = await prisma.series.findFirst({
      where: { title: 'CO Série 11', moduleId: coModule.id }
    })
    if (existing) {
    ...
    const data = JSON.parse(
      fs.readFileSync(path.join(__dirname, '..', 'content', 'co-serie-11.json'), 'utf-8')
    )
    ...
    console.log(`\n✅ ${count} questions insérées — CO Série 11.`)
