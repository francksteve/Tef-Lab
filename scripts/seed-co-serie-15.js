// Script to seed CO Série 15 into the database
// Run: node scripts/seed-co-serie-15.js
...
async function main() {
  ...
  try {
    ...
    const existing = await prisma.series.findFirst({
      where: { title: 'CO Série 15', moduleId: coModule.id }
    })
    if (existing) {
    ...
    const data = JSON.parse(
      fs.readFileSync(path.join(__dirname, '..', 'content', 'co-serie-15.json'), 'utf-8')
    )
    ...
    console.log(`\n✅ ${count} questions insérées — CO Série 15.`)
