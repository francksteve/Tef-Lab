// Script to seed CO Série 5 into the database
// Run: node scripts/seed-co-serie-5.js
...
async function main() {
  ...
  try {
    ...
    const existing = await prisma.series.findFirst({
      where: { title: 'CO Série 5', moduleId: coModule.id }
    })
    if (existing) {
      console.log(`⚠️  CO Série 5 already exists (id: ${existing.id}). Deleting and recreating...`)
    ...
    const data = JSON.parse(
      fs.readFileSync(path.join(__dirname, '..', 'content', 'co-serie-5.json'), 'utf-8')
    )
    ...
    console.log(`\n✅ ${count} questions insérées — CO Série 5.`)
